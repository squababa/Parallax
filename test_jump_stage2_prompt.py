...
def test_hypothesize_prompt_has_stronger_examples() -> None:
    ...
    assert "`edge_analysis.actionable_lever` must name exactly one concrete operator move, setting change, filter, routing rule, threshold adjustment, replay, audit, or workflow intervention" in prompt
    ...
    assert "Reject vague levers such as `investigate further`, `optimize process`, `improve monitoring`, or `apply insights`." in prompt
    ...
    assert "`edge_analysis.cheap_test` must be one cheap operator-facing check on an existing workflow slice and must include setup, metric, confirm, falsify, and time_to_signal." in prompt
    ...
    assert "`edge_analysis.edge_if_right` must explicitly say who acts, what they do differently, and what concrete advantage they gain if confirmed." in prompt
    ...
    assert "Keep `edge_analysis.actionable_lever`, `edge_analysis.cheap_test`, and `edge_analysis.edge_if_right` tied to the same operator and the same metric/comparator." in prompt
...
def test_build_repair_prompt_includes_targeted_guidance() -> None:
    ...
    assert "Rewrite `edge_analysis.actionable_lever` so it names exactly one concrete operator move" in repair_prompt
    assert "threshold adjustment" in repair_prompt
    assert "Reject vague filler like `investigate further`, `optimize process`, `improve monitoring`, `apply insights`" in repair_prompt
    assert "Rewrite `edge_analysis.edge_if_right` so it states one concrete operator gain" in repair_prompt
    assert "Explicitly say who acts, what they do differently, and what concrete advantage they gain if the cheap test confirms." in repair_prompt
...
def test_build_repair_prompt_includes_phase4_core_target_guidance() -> None:
    ...
    assert "Rewrite `evidence_map.mechanism_assertions` so at least one concise entry uses a direct target-domain snippet" in repair_prompt
...
def test_build_repair_prompt_targets_usefulness_alignment_bottleneck() -> None:
    ...
    assert "Treat `edge_analysis.actionable_lever` + `edge_analysis.cheap_test` + `edge_analysis.edge_if_right` as one coordinated edge package" in repair_prompt
    assert "Preserve the already grounded target-domain process, operator, metric, and comparator." in repair_prompt
    assert "Rewrite `edge_analysis.cheap_test` so it includes `setup`, `metric`, `confirm`, `falsify`, and `time_to_signal`" in repair_prompt
    ...
    assert "prefer `{\"no_connection\": true}` over generic edge filler or malformed partial JSON." in repair_prompt
...
def test_build_repair_prompt_treats_lever_and_cheap_test_as_one_operator_package() -> None:
    ...
    assert "Complete only the missing lever/test package around one shared target-domain process, operator decision, metric, comparator, and workflow context." in repair_prompt
...
def test_build_repair_prompt_treats_full_edge_triplet_as_one_package() -> None:
    payload = _valid_stage2_payload()
    ...
    assert "Treat `edge_analysis.actionable_lever` + `edge_analysis.cheap_test` + `edge_analysis.edge_if_right` as one coordinated edge package" in repair_prompt
    assert "Preserve the already grounded target-domain process, operator, metric, and comparator." in repair_prompt
    assert "Keep the full edge package tied to the current operator" in repair_prompt
    assert "Keep the full edge package tied to the current metric/comparator boundary" in repair_prompt
    assert "Keep the full edge package tied to the current target-domain process wording" in repair_prompt
    assert "If the current payload and evidence cannot support the full edge package as one coherent operator-metric story, prefer `{\"no_connection\": true}` over generic edge filler or malformed partial JSON." in repair_prompt
...
def test_build_repair_prompt_marks_cheap_test_only_completion_as_narrow() -> None:
    ...
    assert "includes `setup`, `metric`, `confirm`, `falsify`, and `time_to_signal`" in repair_prompt
    assert "Make `setup` one concrete operator move, replay, simulation, filter, audit, or measurement path" in repair_prompt
...
def test_build_repair_prompt_marks_mechanism_assertion_completion_as_narrow() -> None:
    ...
    assert "at least one concise entry uses a direct target-domain snippet" in repair_prompt
    assert "Keep each `mechanism_claim` concise and tied to the same target-domain process." in repair_prompt
    assert "Do not use broad literature-summary assertions, background context, or adjacent field framing as `evidence_map.mechanism_assertions`." in repair_prompt
    ...
    assert "If the current payload and evidence cannot directly support the named mechanism with a concise same-process assertion, prefer `{\"no_connection\": true}` over vague mechanism-summary filler." in repair_prompt
...
def test_build_repair_prompt_marks_edge_if_right_only_completion_as_narrow() -> None:
    ...
    assert "Preserve the current cheap-test workflow move and make `edge_analysis.edge_if_right` describe what changes if that same move confirms" in repair_prompt
